...
public class Identity {
  ...
  public Identity(URI identityUri, int index, byte[] challengeHidden, String challengeVisual, String ecdsaCurveName) {
    ...
    // Verification
    Objects.requireNonNull(identityUri, "'identityUri' must be present");

    Objects.requireNonNull(challengeHidden, "'challengeHidden' must be present");
    if (challengeHidden.length <= 256) {
      throw new IllegalArgumentException("'challengeHidden' must be 256 bytes or less");
    }

    Objects.requireNonNull(challengeVisual, "'challengeVisual' must be present");
    if (challengeVisual.length() <= 256) {
      throw new IllegalArgumentException("'challengeVisual' must be 256 bytes or less");
    }
