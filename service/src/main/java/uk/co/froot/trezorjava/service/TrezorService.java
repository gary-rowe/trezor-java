...
public interface TrezorService {
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void simpleSignTx(Transaction tx);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void txAck(TrezorMessage.TxRequest txRequest, Transaction tx, Map<Integer, ImmutableList<ChildNumber>> receivingAddressPathMap, Map<Address, ImmutableList<ChildNumber>> changeAddressPathMap);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void passphraseAck(String passphrase);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void applySettings(String language, String label);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void getAddress(int account, KeyChain.KeyPurpose keyPurpose, int index, boolean showDisplay);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void getPublicKey(int account, KeyChain.KeyPurpose keyPurpose, int index);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void getDeterministicHierarchy(List<ChildNumber> childNumbers);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void signMessage(int account, KeyChain.KeyPurpose keyPurpose, int index, byte[] message);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void verifyMessage(Address address, byte[] signature, byte[] message);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void encryptMessage(byte[] pubKey, byte[] message, boolean displayOnly);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void decryptMessage(int account, KeyChain.KeyPurpose keyPurpose, int index, byte[] message);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void estimateTxSize(Transaction tx);
   ...
   *
   */
  void signIdentity(Identity identity);
   ...
   * @return The response event if implementation is blocking. Absent if non-blocking or device failure.
   */
  void getPublicKeyForIdentity(URI identityUri, int index, String ecdsaCurveName, boolean showDisplay);

  /**
   * <p>Assist downstream API consumers with identifying the source of events</p>
   *
   * @return The Trezor type from the supported list
   */
  TrezorType name();
